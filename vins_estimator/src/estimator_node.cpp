...
/**
 * @brief 将vins估计器复位，清空buf，优化器复位，计数器清零的功能
 * 
...
void process()
{
    while (true)    // 这个线程是会一直循环下去
    {
        ...
        // 基于范围的for循环，这里就是遍历每组image imu组合
        for (auto &measurement : measurements)
        {
            ...
            if (relo_msg != NULL)   // 有效回环信息
            {
                ...
                double frame_stamp = relo_msg->header.stamp.toSec();    // 回环帧对应的当前帧的时间戳
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                ...
                double x = img_msg->points[i].x;    // 去畸变后归一化后的相机坐标
