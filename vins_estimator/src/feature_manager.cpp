...
vector<pair<Vector3d, Vector3d>> FeatureManager::getCorresponding(int frame_count_l, int frame_count_r)
{
    ...
    for (auto &it : feature)
    {
        ...
        if (it.start_frame <= frame_count_l && it.endFrame() >= frame_count_r)
        {
            ...
            corres.push_back(make_pair(a, b));  // 返回相机归一化坐标系下的坐标对
        }
    }
    return corres;
}
...
/**
 * @brief 利用观测到该特征点的所有关键帧来三角化特征点
 *        在第一个观察到这个特征点的关键帧相机坐标系下的深度值
 * 
 * @param[in] Ps 
 * @param[in] tic 
 * @param[in] ric 
...
void FeatureManager::removeBackShiftDepth(Eigen::Matrix3d marg_R, Eigen::Vector3d marg_P, Eigen::Matrix3d new_R, Eigen::Vector3d new_P)
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        {
            Eigen::Vector3d uv_i = it->feature_per_frame[0].point;    // 取出特征点在第0帧归一化相机坐标系下的坐标
            ...
            {
                Eigen::Vector3d pts_i = uv_i * it->estimated_depth; // 原来第0帧相机坐标系下的实际坐标
                Eigen::Vector3d w_pts_i = marg_R * pts_i + marg_P;  // 原来第0帧相机系在世界坐标系下的坐标，把pts_i相机系坐标转到世界坐标系下
...
/**
 * @brief 还没初始化结束，此时还没进行视觉惯性对齐，没有地图点深度准确计算结果，因此相比刚才，不进行地图点新的深度的换算
 * 
 */
