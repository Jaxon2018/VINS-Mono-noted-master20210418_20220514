 ...
 * 
 * @param[in & out] R_initial 上一帧的位姿 & 当前帧的位姿
 * @param[in & out] P_initial 
 * @param[in] i 	  当前帧的索引
			  ...
			  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	// Step 2 考虑上一步中有些特征点不能被最后一帧看到而无法三角化出来，因此，fix枢纽帧，利用枢纽帧与枢纽帧后的图像帧到最后一帧进行补充特征点三角化
...
*/
	...
	for (int i = 0; i < frame_num; i++)
	{
		...
		// 这些都是待优化的参数块
		// 5.1需要(该加的参数快)优化的变量转成ceres的double数组；
		...
		// 5.3由于是单目视觉slam，有七个自由度不可观，因此，fix一些参数块避免在零空间漂移
	...
	// 5.4只有视觉重投影构成约束，因此遍历所有的特征点，构建约束
	for (int i = 0; i < feature_num; i++)
	{
		...
		{
			int l = sfm_f[i].observation[j].first;  //第几帧中被观测到的
