...
void ResidualBlockInfo::Evaluate()
{
    ...
    std::vector<int> block_sizes = cost_function->parameter_block_sizes();  // 确定相关的参数块数目和维数
    raw_jacobians = new double *[block_sizes.size()];   // ceres接口都是double数组，因此这里给雅克比准备数组，数组维数等于参数块数目
    jacobians.resize(block_sizes.size());
    // 这里就是把jacobians每个matrix地址赋给raw_jacobians，然后把raw_jacobians传递给ceres的接口，这样计算结果直接放进了对应jacobians的matrix里
    for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
    {
        // 整体残差关于每一个参数块的雅克比：残差维数(预积分15；重投影2)×参数块维数(位姿7，速度零偏9；位姿7，外参7，特征点逆深度1)
        jacobians[i].resize(cost_function->num_residuals(), block_sizes[i]);    // 雅克比矩阵大小 残差×该参数块维数
        raw_jacobians[i] = jacobians[i].data();
        //dim += block_sizes[i] == 7 ? 6 : block_sizes[i];
    }
    // 调用各自重载的接口计算残差和雅克比
    // 计算的结果通过raw_jacobians的指针放在了jacobians[i]的Eigen::Matrix对象里去；
    // residuals.data()取Eigen::VectorXd对象的指针
...
{
    factors.emplace_back(residual_block_info);  // 残差块信息收集起来

    std::vector<double *> &parameter_blocks = residual_block_info->parameter_blocks;    // 和该约束相关的参数块
    std::vector<int> parameter_block_sizes = residual_block_info->cost_function->parameter_block_sizes();   // 得到每个残差块的参数块数目和维数
    ...
    for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size()); i++)
    {
        ...
        // 这里是个unordered map增删改查时间复杂度为O(1)，避免重复添加
...
/**
 * @brief 将各个残差块计算残差和雅克比，同时备份边缘化时刻所有相关的参数块内容(残差块的参数块内容=>边缘化参数块内容)
 * 
 */
void MarginalizationInfo::preMarginalize()
{
    for (auto it : factors)
    {
        ...
        std::vector<int> block_sizes = it->cost_function->parameter_block_sizes();  // 得到每个残差块的参数块数目和维数
        for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
        {
            long addr = reinterpret_cast<long>(it->parameter_blocks[i]);    // 得到该参数块的地址，ResidualBlockInfo的成员变量
            int size = block_sizes[i];  // 该参数块维数
            // 把各个参数块都备份起来，使用unordered map避免重复参数块，之所以备份，是为了后面的状态保留
            if (parameter_block_data.find(addr) == parameter_block_data.end())
            {
                double *data = new double[size]; // 新申请一个double数组，用于备份存储参数块的内容
                // 深拷贝
                memcpy(data, it->parameter_blocks[i], sizeof(double) * size);
                parameter_block_data[addr] = data;  // 地址->备份参数块内容的内存地址，MarginalizationInfo的成员变量
            }
        }
    }
}
...
void* ThreadsConstructA(void* threadsstruct)
{
    ...
    for (auto it : p->sub_factors)
    {
        ...
        for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++)
        {
            ...
            if (size_i == 7)
                size_i = 6;
            // 之前preMarginalize()已经算好了各个残差和雅克比，这里取出来
...
void MarginalizationInfo::marginalize()
{
    ...
    for (auto &it : parameter_block_idx)
    {
        ...
        pos += localSize(parameter_block_size[it.first]);   // 因为要进行求导，因此大小是local size，具体一点就是使用李代数-只涉及位姿参数块
    }
...
std::vector<double *> MarginalizationInfo::getParameterBlocks(std::unordered_map<long, double *> &addr_shift)
{
    ...
    for (const auto &it : parameter_block_idx)  // 遍历边缘化相关的每个参数块
    {
        if (it.second >= m) // 如果是留下来的，说明后续会对其形成约束
        {
            ...
            // marginalize()函数刚开始构造的状态量parameter_block_idx；
            keep_block_idx.push_back(parameter_block_idx[it.first]);    // 留下来的在原向量中的排序local Size
            ...
            // 被边缘化掉的参数块对留下来的参数块形成约束，被约束的参数块统一的保存在keep_block_addr中
        }
    }
    // 留下来的边缘化后的参数块总大小
...
{
    int cnt = 0;
    for (auto it : marginalization_info->keep_block_size)   // keep_block_size表示上一次边缘化留下来的参数块的大小,global size
    {
...
bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    int m = marginalization_info->m;    // 上次边缘化时被margin掉的残差块总和
    Eigen::VectorXd dx(n);  // 用来存储残差
    // 遍历所有的剩下的有约束的残差块,计算残差
    for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++)
    {
        int size = marginalization_info->keep_block_size[i];
        int idx = marginalization_info->keep_block_idx[i] - m;  // idx起点统一到0
        Eigen::VectorXd x = Eigen::Map<const Eigen::VectorXd>(parameters[i], size); // 当前滑窗里调整之后的参数块的值
        Eigen::VectorXd x0 = Eigen::Map<const Eigen::VectorXd>(marginalization_info->keep_block_data[i], size); // 边缘化时参数块的值
        if (size != 7)
