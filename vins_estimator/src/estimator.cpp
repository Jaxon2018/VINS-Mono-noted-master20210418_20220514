...
/**
 * @brief Step 1 将特征点信息加到f_manager这个特征点管理器中，同时进行是否关键帧的检查 
 *        Step 2： 外参初始化
 *        Step 3： VIO初始化
 *        Step 4： 非线性优化求解VIO
 *        Step 5： 滑动窗口
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    {
        if (frame_count == WINDOW_SIZE) // frame_count滑窗中帧索引，frame_count == WINDOW_SIZE滑窗满了，有足够的帧数
        {
            bool result = false;
            // 要有可信的外参值，同时距离上次初始化不成功至少相邻0.1s
            // Step 3： VIO初始化
            // 距离上一次VIO初始化失败的时间间隔大于0.1秒
...
bool Estimator::initialStructure()
{
    ...
    // 2.1遍历所有的特征点，保存每个特征点的信息
    for (auto &it_per_id : f_manager.feature)
    {
        int imu_j = it_per_id.start_frame - 1;  // 这个跟imu无关，就是存储观测特征点的帧的索引
        SFMFeature tmp_feature; // 后续做sfm需要的状态量，每一个id的特征点都会维护一个结构体
        tmp_feature.state = false; // 该特征点还未三角化或三角化未成功，true三角化成功
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            // 保存特征点在每一帧中的观测：观测到特征点的帧在滑窗中的索引以及在该帧下的归一化相机坐标系下的坐标
...
bool Estimator::visualInitialAlign()
{
    ...
    // 利用观测到该特征点的所有关键帧来三角化特征点,得到
    // 在第一个观察到这个特征点的关键帧c0相机坐标系下的深度值
    ...
    // 5下面开始把所有的状态对齐到第0帧的imu坐标系
    for (int i = frame_count; i >= 0; i--)
        // twi - tw0 = t0i,就是把所有的平移对齐到滑窗中的第0帧，第0帧0平移
        // 此时才恢复枢纽帧系下IMU帧位移的尺度
    ...
    // 所有的P V Q全部对齐到第0帧的同时也对齐到重力方向
    Matrix3d R0 = Utility::g2R(g);  // g是枢纽帧下的重力方向，得到枢纽帧相对世界系下的旋转矩阵R_w_j(R_w_l)，偏航角=0
    double yaw = Utility::R2ypr(R0 * Rs[0]).x();    // Rs[0]实际上是R_j_0(R_l_0), 第0帧(R_w_0)在世界坐标系下的旋转矩阵的yaw角
    R0 = Utility::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;  // 第0帧(R_w_0)yaw赋0；枢纽帧相对世界帧下的旋转矩阵，偏航角为-yaw
    g = R0 * g;
    ...
    {
        Ps[i] = rot_diff * Ps[i];
        Rs[i] = rot_diff * Rs[i];   // 全部对齐到重力下(世界坐标系下)，同时yaw角对齐到第0帧
        Vs[i] = rot_diff * Vs[i];
    }
...
void Estimator::double2vector()
{
    ...
    if(relocalization_info) // 类似进行一个调整, 参数块relo_Pose回环帧在当前帧坐标系中优化调整比较好的一个位姿
    { 
        Matrix3d relo_r;
        Vector3d relo_t;
        // 补偿 fix第一帧的yaw和平移，固定了四自由度的零空间 这部分
        ...
        // T_loop_w * T_w_cur = T_loop_cur 优化后VIO坐标系下当前帧到回环帧的位姿变换
...
{
    if (f_manager.last_track_num < 2)   // 地图点数目是否足够，小于两个视觉就失效了，VIO也失效了
    {
        ROS_INFO(" little feature %d", f_manager.last_track_num);
        //return true;
    }
    if (Bas[WINDOW_SIZE].norm() > 2.5)  // 零偏是否正常，加速度计零偏大于2.5m/s2
    {
        ROS_INFO(" big IMU acc bias estimation %f", Bas[WINDOW_SIZE].norm());
        return true;
    }
    if (Bgs[WINDOW_SIZE].norm() > 1.0)  // 零偏是否正常，陀螺仪零偏大于1弧度
    {
    ...
    */
    ...
    if ((tmp_P - last_P).norm() > 5)    // 两帧之间运动是否过大, 0.1秒运动5米
    {
        ROS_INFO(" big translation");
        return true;
    }
    if (abs(tmp_P.z() - last_P.z()) > 1)    // 重力方向运动是否过大，0.1秒运动1米
    {
    ...
    if (delta_angle > 50)   // 两帧姿态变化是否过大，0.1秒大于50度
    {
...
void Estimator::optimization()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        vector2double();
        // 关于边缘化有几点注意的地方
        // 1、找到需要被边缘化掉的参数块，这里是地图点，第0帧位姿，第0帧速度零偏
        ...
        // 只有第1个预积分和待边缘化参数块相连
        {
            if (pre_integrations[1]->sum_dt < 10.0)
            {
                ...
                // cost function，定义计算残差和雅克比的公式
                IMUFactor* imu_factor = new IMUFactor(pre_integrations[1]);
                // 残差块信息：cost function（约束），loss function：残差的计算方式，相关联的参数块，待边缘化的参数块的索引
        ...
        // 遍历视觉重投影的约束
        {
            ...
            for (auto &it_per_id : f_manager.feature)
            {
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)
                {
                    ...
                    {
                        ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f, loss_function, // 7,7,7,1
        ...
        // 所有的残差块都收集好了
        TicToc t_pre_margin;
        // 进行预处理，将各个残差块计算残差和雅克比，同时备份边缘化时刻所有相关的参数块内容
        marginalization_info->preMarginalize();
        ROS_DEBUG("pre marginalization %f ms", t_pre_margin.toc());
        
        TicToc t_margin;
        // 边缘化操作, 边缘化处理，并将舒尔补的结果转换成残差和雅克比的形式
        marginalization_info->marginalize();
        ROS_DEBUG("marginalization %f ms", t_margin.toc());
        
        // 即将滑窗，因此记录原参数块对应的新位置地址
        ...
        // 备份边缘化后留下来的被约束的参数块信息，返回这些参数块对应的新地址
        vector<double *> parameter_blocks = marginalization_info->getParameterBlocks(addr_shift);

        if (last_marginalization_info)
            delete last_marginalization_info;
        last_marginalization_info = marginalization_info;   // 本次边缘化的所有信息
        last_marginalization_parameter_blocks = parameter_blocks;   // 代表该次边缘化对某些参数块形成约束，这些参数块在滑窗之后的新地址
        
    }
    else    // 边缘化倒数第二帧
    {
        // 有上一次边缘化的内容/结果，同时，即将被margin掉的倒数第二帧的位姿参数块在上一次边缘化后形成的约束中
...
void Estimator::slideWindowOld()
{
    ...
    if (shift_depth)    // 如果初始化过了，3D点得到比较好的值，被正确的恢复出来了；
    {
        Matrix3d R0, R1;
        Vector3d P0, P1;
        // back_R0 back_P0是被移除的帧的位姿
        R0 = back_R0 * ric[0];  // 被移除的相机的姿态Rwi*Ric=Rwc
