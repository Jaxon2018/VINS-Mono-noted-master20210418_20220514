...
/**
 * @brief 将当前帧的原有描述子依次和回环帧描述子进行匹配，得到匹配结果
 * 
...
/**
 * @brief 通过PNP对当前帧和回环帧是否构成回环进行几何校验
 * 
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
	...
	Matrix3d R_inital;
	Vector3d P_inital;
	Matrix3d R_w_c = origin_vio_R * qic;	   // VIO节点发出的当前帧IMU坐标系下坐标转成相机坐标系
	...
	else
	{
		if (CV_MINOR_VERSION < 2)
			...
			// 考虑到回环帧距离起始帧并不远，使用当前帧位姿作为起始位姿，计算回环帧在当前帧相机坐标系Tcw下的位姿
			// 去完畸变了，所以不考虑D
	...
	PnP_R_old = R_w_c_old * qic.transpose();	// 这是回环帧在VIO坐标系下的位姿，相机系转成IMU系；
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	// 通过描述子来check
	// 将当前帧的原有描述子依次和回环帧额外提取的fast角点描述子进行匹配，得到匹配结果
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		...
		if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)
		{
			...
			loop_info << relative_t.x(), relative_t.y(), relative_t.z(),
					...
					relative_yaw;
			// FAST_RELOCALIZATION与VIO节点有交互，把检测回环成功的回环帧匹配信息通过ROS消息msg_match_points发送给VIO节点
			if(FAST_RELOCALIZATION)
			{
				sensor_msgs::PointCloud msg_match_points;
				msg_match_points.header.stamp = ros::Time(time_stamp); // 当前帧的时间戳赋给回环帧
				// 回环帧2d归一化坐标
				for (int i = 0; i < (int)matched_2d_old_norm.size(); i++)
				{
					...
					p.z = matched_id[i];	// 回环帧对应的VIO坐标系下当前帧地图点的id
