...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    // 将新地图的当前帧位姿通过两个地图的位姿差转换到老地图中，实现地图合并
	...
	if (loop_index != -1)   // 代表找到了有效的回环帧
	{
        ...
        if (cur_kf->findConnection(old_kf)) // 如果确定两者回环
        {
            ...
            // T_old_cur
            // 当前帧与回环帧之间的相对位姿变换。是否是滑窗优化后的VIO坐标系下当前帧到回环帧之间的位姿差？
            relative_t = cur_kf->getLoopRelativeT();
            relative_q = (cur_kf->getLoopRelativeQ()).toRotationMatrix();
            // T_w_old * T_old_cur = T_w_cur,这就是在老帧基础上回环矫正后得到的当前帧的位姿
            ...
            Matrix3d shift_r;
            Vector3d shift_t; 
            // 回环矫正前的当前帧的位姿认为是T_w'_cur：vio_R_cur + vio_P_cur
            // 下面求得是 T_w_cur * T_cur_w' = T_w_w' 
            // 当前帧VIO给的原来位姿和回环检测基于老帧计算的当前帧新的位姿之间的偏移量deltaT(两个坐标系之间的相对位姿变换)
            ...
            {  
                w_r_vio = shift_r;
                w_t_vio = shift_t;
                // 将计算的偏移量T_w_w'加到当前帧VIO给的原来得位姿上，得到在老地图中当前帧的位姿，更新当前帧位姿
                // T_w_w' * T_w'_cur = T_w_cur
                vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
                vio_R_cur = w_r_vio *  vio_R_cur;
                cur_kf->updateVioPose(vio_P_cur, vio_R_cur);    // 更新当前帧位姿
                list<KeyFrame*>::iterator it = keyframelist.begin();
                // 同时把这个序列(新地图)的关键帧的位姿都找到在老地图中的位姿，并更新关键帧的位姿；
                // 新地图关键帧的坐标系统一到老地图的坐标系中
...
*/
    ...
    {
        int min_index = -1;
        // 寻找得分大于0.015的idx最小的那个帧，这样可以尽可能调整更多帧(最小idx帧到当前帧)的位姿
        for (unsigned int i = 0; i < ret.size(); i++)
        {
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            {
                if ((*it)->index < first_looped_index)  // idx小于最早回环帧的关键帧不进行优化
                    continue;
                (*it)->local_index = i; // 这个是在本次优化中的idx
                Quaterniond tmp_q;
                Matrix3d tmp_r;
                Vector3d tmp_t;
                (*it)->getVioPose(tmp_t, tmp_r);    //  得到关键帧的位姿
                tmp_q = tmp_r;
                t_array[i][0] = tmp_t(0);
                t_array[i][1] = tmp_t(1);
                t_array[i][2] = tmp_t(2);
                ...
                for (int j = 1; j < 5; j++)
                {
                  if (i - j >= 0 && sequence_array[i] == sequence_array[i-j])
                  {
                    ...
                    ceres::CostFunction* cost_function = FourDOFError::Create( relative_t.x(), relative_t.y(), relative_t.z(),
                                                   relative_yaw, euler_conncected.y(), euler_conncected.z()); // i-j帧欧拉角pitch,roll
                    // 对i-j帧和第i帧都成约束
                ...
                if((*it)->has_loop)
                {
                    ...
                    Vector3d relative_t;
                    relative_t = (*it)->getLoopRelativeT(); // 得到滑窗优化后的VIO坐标系下当前帧到回环帧的相对位姿
            ...
            */
            m_keyframelist.lock();
            i = 0;
            // 将最早回环帧与当前帧之间的所有位姿恢复成四自由度优化后的位姿
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
            ...
            cur_kf->getPose(cur_t, cur_r);  // 最新优化后的当前帧的位姿
            cur_kf->getVioPose(vio_t, vio_r);   // VIO的当前帧的位姿
            m_drift.lock();
            // 计算当前帧的VIO位姿和优化后位姿差 T_w_cur' - T_w_cur = T_cur'_cur
...
void PoseGraph::savePoseGraph()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        Quaterniond VIO_tmp_Q{(*it)->vio_R_w_i}; // 滑窗输入的VIO的位姿
        Quaterniond PG_tmp_Q{(*it)->R_w_i};      // 回环检测修正过得全局一致性更好的位姿
        ...
        fprintf (pFile, " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f %f %d\n",(*it)->index, (*it)->time_stamp, 
                                    ...
                                    (int)(*it)->keypoints.size()); // 该关键帧特征点的数量
        // 存储这一帧的特征点和描述子
...
{
    KeyFrame* kf = getKeyFrame(index); // 得到当前帧
    kf->updateLoop(_loop_info); // 更新当前帧的滑窗优化后的与回环帧的相对位姿信息
    if (abs(_loop_info(7)) < 30.0 && Vector3d(_loop_info(0), _loop_info(1), _loop_info(2)).norm() < 20.0)
    {
        if (FAST_RELOCALIZATION)    // 肯定只有这种情况下触发
        {
            KeyFrame* old_kf = getKeyFrame(kf->loop_index); // 得到回环帧
            Vector3d w_P_old, w_P_cur, vio_P_cur;
            Matrix3d w_R_old, w_R_cur, vio_R_cur;
            old_kf->getPose(w_P_old, w_R_old);       // T_w_loop
            kf->getVioPose(vio_P_cur, vio_R_cur);    // T_w'_cur

            Vector3d relative_t;
            Quaterniond relative_q;
            // 得到T_loop_cur 优化后VIO坐标系下当前帧到回环帧的位姿变换
            relative_t = kf->getLoopRelativeT();
            relative_q = (kf->getLoopRelativeQ()).toRotationMatrix();
            // T_w_loop * T_loop_cur = T_w_cur 回环帧位姿*优化后VIO坐标系下当前帧到回环帧的位姿变换=相对回环帧的当前帧比较准确的位姿
            w_P_cur = w_R_old * relative_t + w_P_old;
            w_R_cur = w_R_old * relative_q;
            double shift_yaw;
            Matrix3d shift_r;
            Vector3d shift_t;
            // T_w_cur * (T_w'_cur)-1 = T_w_w’,也就是两个地图sequence坐标系的差值T_w_w’
            shift_yaw = Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();
            shift_r = Utility::ypr2R(Vector3d(shift_yaw, 0, 0));
            shift_t = w_P_cur - w_R_cur * vio_R_cur.transpose() * vio_P_cur; 
